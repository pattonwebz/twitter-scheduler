...
(function (global, factory) {
	...
	var extractCashtagsWithIndices = function (text) {
		...
		text.replace(
			validCashtag, function (match, before, dollar, cashtag, offset, chunk) {
	...
	var extractUrlsWithIndices = function extractUrlsWithIndices(text) {
		...
		var _loop = function _loop() {
			...
			if ( ! protocol) {
				...
				domain.replace(
					validAsciiDomain, function (asciiDomain) {
	...
	var extractHashtagsWithIndices = function extractHashtagsWithIndices(text, options) {
		...
		text.replace(
			validHashtag, function (match, before, hash, hashText, offset, chunk) {
	...
	var extractMentionsOrListsWithIndices = function (text) {
		...
		text.replace(
			validMentionOrList, function (match, before, atSign, screenName, slashListname, offset, chunk) {
				var after = chunk.slice( offset + match.length );
	...
	var parseTweet = function parseTweet() {
		...
		var _loop = function _loop(_charIndex) {
			...
			var urlEntity = urlsWithIndices.filter(
				function (_ref) {
